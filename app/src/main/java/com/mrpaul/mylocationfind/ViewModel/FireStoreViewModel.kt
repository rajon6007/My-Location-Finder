package com.mrpaul.mylocationfind.ViewModel
...
class FireStoreViewModel : ViewModel() {
    ...
    fun getAllUsers(context: Context, callback: (List<User02>) -> Unit) {
        usersCollection.get()
            .addOnSuccessListener {
                val userList = mutableListOf<User02>()
                for (document in it) {
                    val userId = document.id
                    val displayName = document.getString("displayName") ?: ""
                    val email = document.getString("email") ?: ""
                    val location = document.getString("location") ?: ""
                    userList.add(User02(userId, displayName, email, location))
                }
                callback(userList)
            }
            .addOnFailureListener { e ->
    ...
    fun getUser(context: Context, userId: String, callback: (User02?) -> Unit) {
        usersCollection.document(userId).get()
            .addOnSuccessListener {
                val user = it.toObject(User02::class.java)
                callback(user)
            }
            .addOnFailureListener { e ->
                Toast.makeText(context,e.message.toString(), Toast.LENGTH_SHORT).show()
